...
function rureraform_builder_save(_object, question_status) {
    ...
    jQuery.ajax({
        ...
        success: function (return_data) {
            ...
            if($('.single-question-layout').length < 1) {
...
function _rureraform_properties_prepare(_object) {
            ...
            switch (rureraform_meta[type][key]['type']) {
                ...
                case 'rurera_image':
                    ...
                    var image_attributes = {
                        ...
                        is_multiple: false,
                        preview_div: 'preview-images-block',
                        hidden_field: '<input value="" type="hidden" name="rureraform-content" id="rureraform-content">',
                    ...
                    var gallery_images_html = loadGalleryHTML(rureraform_escape_html(properties[key]), '<input value="' + rureraform_escape_html(properties[key]) + '" type="text" name="rureraform-content" id="rureraform-content">');
                ...
                case 'select':
                    ...
                    var wrapper_class = (rureraform_meta[type][key]['wrapper_class'] != undefined)? rureraform_meta[type][key]['wrapper_class'] : '';
                    var attr_html = (rureraform_meta[type][key]['attr'] != undefined)? rureraform_meta[type][key]['attr'] : '';
                    html += "<div class='rureraform-properties-item "+wrapper_class+"' data-id='" + key + "'><div class='rureraform-properties-label'><label>" + rureraform_meta[type][key]['label'] + "</label></div><div class='rureraform-properties-tooltip'>" + tooltip_html + "</div><div class='rureraform-properties-content'><div class='rureraform-third'><select name='rureraform-" + key + "' id='rureraform-" + key + "' class='"+field_class+"' "+attr_html+">" + options + "</select></div></div></div>";
                    break;
    ...
    $('.select-conditional select').each(function () {
        handleConditionalSelect(this);
    });
        ...
        if (rureraform_toolbar_tools.hasOwnProperty(rureraform_form_elements[i]["type"])) {
            switch (rureraform_form_elements[i]["type"]) {
                ...
                case "example_text":
                    ...
                    var label_type = '';
                    var label_data = "<div class='example-heading " + label_type + "'><span>" + rureraform_form_elements[i]["content"] + "</span></div>";
                    if(label_type == 'h1' || label_type == 'h2' || label_type == 'h3' || label_type == 'h4' || label_type == 'h5' || label_type == 'h6'){
                        var label_data = "<" + label_type + ">" + rureraform_form_elements[i]["content"] + "</" + label_type + ">";
                    }
                    html += "<div id='rureraform-element-" + i + "' class='rureraform-element-" + i + " rureraform-element quiz-group  rureraform-element-html' data-type='" + rureraform_form_elements[i]["type"] + "'>"+label_data+"</div>";
...
$(document).on('click', '.duplicate-question', function () {
    ...
    rureraform_dialog_open({
        ...
        ok_function: function (e) {
            ...
            jQuery.ajax({
                type: "GET",
                url: '/admin/questions-generator/'+question_id+'/duplicate_question',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                data: {"question_id": question_id},
...
function handleConditionalSelect(select) {
    ...
    // Normalize child classes
    let childClasses = $select.attr('data-child_classes');
    let conditionalFields = $select.attr('data-conditional_fields');

    if (!childClasses || !conditionalFields) return;

    // Convert childClasses to array
    try {
        childClasses = JSON.parse(childClasses);
    } catch (e) {
        childClasses = [childClasses];
    }

    // Convert conditionalFields to object
    try {
        conditionalFields = JSON.parse(conditionalFields);
    } catch (e) {
        conditionalFields = {};
    }

    // 1️⃣ Hide all child elements
    childClasses.forEach(cls => {
        $(cls).addClass('rurera-hide');
    });

    // 2️⃣ Show conditional fields if matched
    if (conditionalFields[selectedValue]) {
        let targets = conditionalFields[selectedValue];
...
$(document).ready(function() {
    ...
    $(document).on('change', '.select-conditional select', function () {
        handleConditionalSelect(this);
    });
...
function loadGalleryHTML(file_url, input_data){
    var response_url = '';
    response_url += '<li>'+input_data+'<img src="'+file_url+'" style="width:80px;"><i class="fa fa-times remove-gallery-image"></i></li>';
    return response_url;
}

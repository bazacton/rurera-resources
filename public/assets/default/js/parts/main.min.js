...
$(document).on("click", ".subscription-school-modal", function (e) {
    ...
    $(document).on("click", ".package-selection", function (e) {
        ...
        $('.package-register-form-fields')
            .find('input')
            .each(function () {

                let fullName = $(this).attr('name'); // e.g. userRegisterForm[first_name]
                let value = $(this).val();

                // 👇 Extract only the key inside []
                let match = fullName.match(/userRegisterForm\[(.*?)\]/);

                if (!match) return; // skip if not matched

                let key = match[1]; // first_name, last_name, etc.
        ...
        // 👇 Add extra values
        urf.selected_package = t;
        urf.subscribed_for = n;
        urf.user_id = a;

        console.log(urf);

        // 👇 Send as JSON (important)
        r.append("userRegisterForm", JSON.stringify(urf));
    ...
    $(document).on("click", ".process-payment", function (e) {
        ...
        var t = $(this).attr("data-subscribed_for"),
            a = $(".coupon_code").val(),
            selectedPackage = $(this).attr("data-selected_package"),
...
function scroll_cb() {
    var HEADER_OFFSET = 80; // ← set to your fixed header height
    var scrollTop = $(window).scrollTop() + HEADER_OFFSET;
...
console.log('sdfsdfsdfsfsdfsdfsdf');
$(document).on("click", ".rurera-selectable-options li", function (e) {
    console.log('rurera-selectable-options li');
...
function rurera_modal_alert(msg_type, msg_title, confirmButton = true, html_template = '', afterConfirmFunc = '') {

    if (html_template === '') {
        html_template = '<h3 class="font-20 text-center text-dark-blue">' + msg_title + '</h3>';
    }
    ...
    Swal.fire({
        ...
        // Only when Continue Working is clicked
        if (result.dismiss === Swal.DismissReason.cancel) {
            if (typeof afterContinue === "function") {
                afterContinue();
            }
        }
...
function selected_topics_tags(){
    ...
    $(".topic-selection-check:checked").each(function () {
        ...
        var selected_topic_html =
            '<button class="chip selected-topic-chip" type="button" data-id="' + topic_id + '">' +
            topic_title +
            '<i class="fa fa-times unlink-topic-chip">×</i></button>';
...
$(document).on("click", ".unlink-topic-chip", function () {

    var topic_id = $(this).closest('.selected-topic-chip').attr('data-id');
    $('.topic-selection-check[value="'+topic_id+'"]').prop('checked', false);
    $(this).closest('.selected-topic-chip').remove();
    $(".topic-selection-check").change();
});
...
$(document).ready(function () {
    ...
    if ($('#featureSwiper').length) {
        ...
        function update(swiper) {
            if (!swiper.el.matches('#featureSwiper')) return;
...
$(document).on('change', '.subscribed_for-field', function (e) {
    ...
    $('.package-selection').removeClass('disabled-style');
    $('.package-selection').removeClass('disabled-div');



    $(".packages-prices").each(function(){
        var currency_sign = $(this).attr('data-currency_sign');
        var package_price = $(this).attr('data-package_price');
        var package_id = $(this).attr('data-package_id');
        var selected_package = $(this).attr('data-selected_package');
        var user_subscribed_for = $(this).attr('data-user_subscribed_for');

        console.log('package_id='+package_id);
        console.log('selected_package='+selected_package);
        console.log('user_subscribed_for='+user_subscribed_for);
        console.log('package_month='+package_month);

...
(function () {
  var sections = [];
  var ticking = false;
  var OFFSET = 50; // same as your "n - 50"

  function buildSections() {
    sections = $("h2[id], ul[id], h3[id]")
      .map(function () {
        var $el = $(this);
        return {
          id: $el.attr("id"),
    ...
    // Ensure sorted (important if DOM order is odd)
    sections.sort(function (a, b) { return a.top - b.top; });
  }

  function setActive(id) {
    var $link = $(".single-post-nav li a[href='#" + id + "']");
    if (!$link.length) return;

    var $current = $(".single-post-nav li a.current");
    // Avoid unnecessary DOM class toggles
    if ($current.length && $current[0] === $link[0]) return;
  ...
  function updateActive() {
    ...
    // Find the section containing the current scrollTop
    // (linear scan is usually fine; can be optimized to binary search if needed)
    for (var i = 0; i < sections.length; i++) {
      if (scrollTop >= sections[i].top && scrollTop < sections[i].bottom) {
        setActive(sections[i].id);
        return;
      }
    }

    // Optional: if you want "last section" active when below it
    if (sections.length && scrollTop >= sections[sections.length - 0].top) {
      setActive(sections[sections.length - 0].id);
    }
  }
  ...
  // Rebuild when layout may change
  ...
  // If images/fonts load later and shift layout, rebuild after load as well

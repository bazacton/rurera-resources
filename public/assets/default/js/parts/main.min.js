...
$(document).on("click", ".subscription-school-modal", function (e) {
    ...
    $(document).on("click", ".package-selection", function (e) {
        ...
        var t = $(this).attr("data-id"),
            ...
            urf = $('input[name="userRegisterForm[]"]').val(),
            r = new FormData($(".package-register-form")[0]);

        // 👇 Create object of all form fields
        var userRegisterForm = {};
        console.log('urf');
        console.log(urf);

        $(".package-register-form-fields").serializeArray().forEach(function(field) {

            if (userRegisterForm[field.name]) {
                // Already exists → convert to array
                if (!Array.isArray(userRegisterForm[field.name])) {
                    userRegisterForm[field.name] = [userRegisterForm[field.name]];
                }
                userRegisterForm[field.name].push(field.value);
            } else {
                userRegisterForm[field.name] = field.value;
            }
        ...
        // 👇 Append your extra fields into BOTH
        userRegisterForm.selected_package = t;
        userRegisterForm.subscribed_for = n;
        userRegisterForm.user_id = a;

        r.append("selected_package", t);
        r.append("subscribed_for", n);
        r.append("userRegisterForm", urf);
        r.append("user_id", a);

        // 👇 IMPORTANT: append full object as JSON string
        r.append("userRegisterForm", JSON.stringify(userRegisterForm));

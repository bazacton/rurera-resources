...
<div class="sats-listing-item">
    ...
    <div class="text-right {{$button_class}}">
        ...
        @if (auth()->check() && auth()->user()->isUser())
            <a href="/{{isset($url_slug)? $url_slug : 'sats'}}/{{$rowObj->quiz_slug}}" class="rurera-list-btn font-14 {{ subscriptionCheckLink('bookshelf') }}">{{$button_label}} -- {{auth()->user()->id}} == {{$rowObj->id}}</a>
        @endif
    </div>
</div>

...
if( !empty( $emojisArray ) ){
	...
	foreach( $emojisArray as $emojiCode){
		if( $emojiCode != ''){
			$emoji_response .= '<button type="button" id="'.$emojiCode.'" class="emoji-icon" aria-label="Select emoji .$emojiCode."><img src="/assets/default/svgs/emojis/icon'.$emojiCode.'.svg" alt="emoji image"></a>';
		}
	}
	$emoji_response .= '</div>';
}

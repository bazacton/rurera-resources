        ...
        <table class="topics-table mt-0">
            ...
            @if(!empty($listingData))
                @foreach($listingData as $listingObj)
                    @php $topic_completion = isset($listingObj->performance)? $listingObj->performance : 0;
                        $last_activity = isset($listingObj->last_activity)? $listingObj->last_activity : '-';
                        $no_of_attempts = isset($listingObj->no_of_attempts)? $listingObj->no_of_attempts : 0;
                        $sas_accuracy = isset($listingObj->sas_accuracy)? $listingObj->sas_accuracy : 0;
                        $completion_title = 'Not Started';
                        $completion_title = ($topic_completion > 0)? 'Practice Needed' : $completion_title;
                        $completion_title = ($topic_completion > 39)? 'Good' : $completion_title;
                        $completion_title = ($topic_completion > 59)? 'Very Good' : $completion_title;
                        $completion_title = ($topic_completion > 79)? 'Excellent' : $completion_title;
                    @endphp
                    <tr class="listing-data-row" data-level_type="{{$completion_title}}" data-parent_id="{{isset($listingObj->parent_id)? $listingObj->parent_id : 0}}">
                        ...
                        <td data-th="Last seen">{{$no_of_attempts}}</td>
                        ...
                        <td data-th="Performance">
                            ...
                            <div class="percent-holder">
                                <div class="chapter_percent circle-blue" data-percent="{{$topic_completion}}">
                        ...
                        <td data-th="Last seen">
                            {{$sas_accuracy}}
                            <div class="percent-holder">
                                <div class="chapter_percent circle-blue" data-percent="{{$sas_accuracy}}">
                                    <div class="circle_inner">
                                        <div class="round_per"></div>
                                    </div>
                                </div>

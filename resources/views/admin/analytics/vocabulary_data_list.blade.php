        ...
        <tr class="topic-heading-top">
            ...
            @if($vocabulary_list->count() > 0)
                @foreach($vocabulary_list as $vocabularyObj)
                    <th class="font-14" colspan="4"> {{$vocabularyObj->getTitleAttribute()}}</th>
                @endforeach
            @endif
        </tr>
        @if($studentsList->count() > 0)
            @foreach($studentsList as $studentObj)
                <tr class="topic-heading-top">
                    ...
                    @while($topic_counter <= $vocabulary_list->count())
                ...
                <tr>
                    ...
                    @if($vocabulary_list->count() > 0)
                        @foreach($vocabulary_list as $vocabularyObj)
                            ...
                            <td>{{$vocabularyObj->parentResults->where('status', 'passed')->count()}}</td>
                            <td class="">

									...
									<div class="col-12 col-md-12">
										...
										<div class="col-12 col-md-12">
											<div class="create-question-fields-block d-flex align-items-center mb-30">
                                                ...
                                                <a href="/panel/questions/{{isset( $questionObj->id )? $questionObj->id : 0 }}/start" target="_blank">
                                                    View Question
                                                </a>
											</div>
										</div>
																					...
																					<div class="media-body">
																						<div class="d-flex justify-content-between align-items-center lms-card-info">
																							...
																							<div class="log_details">
																								<span class="badge">{{$logObj->action_type}}</span>
																							</div>
																						</div>
																					</div>
																				</div>
...
@include('admin.questions_bank.interactive_elements.flexible_table_builder')
@include('admin.questions_bank.interactive_elements.number_line_tool')
...
function extractLatex(text) {
    const regex = /(\\dfrac|\\frac|\\sqrt|\\sum|\\int|\\lim|\\begin|\\\(|\\\[|\$\$|\$).*?(\\\)|\\\]|\$\$|\$)/gs;
    return text.match(regex) || [];
}
function escapeLatexSpecialChars(latex) {
    return latex.replace(/#/g, '\\#');
}
function convertLatexOnly(input) {
    const latexBlocks = extractLatex(input);

    latexBlocks.forEach(latex => {
        const escaped = escapeLatexSpecialChars(latex);
...
${escaped}
        ...
        input = input.replace(latex, wrapped);
    });

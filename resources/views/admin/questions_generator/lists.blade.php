...
@push('libraries_top')
    ...
    <style>
        ...
        /* Small nested modal */
        .mock-exam-modal.modal-sm2 .modal-dialog {max-width: 720px; margin: 1.75rem auto;}
...
@section('content')
                                ...
                                <table class="table table-striped font-14" id="myTable">
                                    ...
                                    @foreach($QuestionsBulkLists as $QuestionsBulkListObj)
                                        ...
                                        if($QuestionsBulkListObj->quiz_id > 0){
                                            $quizObj = $QuestionsBulkListObj->quiz;

                                            $title = isset($quizObj->id)? $quizObj->getTitleAttribute().' ' : '';

                                        }
                                        @endphp

                                        <tr>
                                            <td data-id="category" class="text-left">
                                                @if($title != '')
                                                    {{$title}}
                                                    <br>
                                                    <small>
                                                        {{ (isset($QuestionsBulkListObj->category->id))? $QuestionsBulkListObj->category->getTitleAttribute() : '-' }} /
    ...
    <div id="questions_bulk_list_form" class="questions_bulk_list_form modal fade" role="dialog" data-backdrop="static">
            ...
            <script>
                $(document).ready(function () {
                    ...
                    $(document).on('change', '.mock-practice-data', function () {
                        ...
                        var category_id = $(".mock-category-field").val();
                        var quiz_id = 0;
            ...
            <script type="text/javascript">
                ...
                $(document).ready(function () {
                    ...
                    $('body').on('change', '.assignment_subject_check', function (e) {
                        ...
                        var category_id = $('.mock-category-field').val();
                        var bulk_id = $('input[name="bulk_id"]').val();
                        var thisObj = $(this);
                        ...
                        jQuery.ajax({
                            ...
                            data: {"bulk_id": bulk_id, "subject_id": subject_id, 'category_id': category_id},
                            success: function (return_data) {
                ...
                window.mockExam = (function($) {
                    ...
                    function renderSections() {
                        ...
                        state.sections.forEach((sec, idx) => {
                            ...
                            var fields_response = '<span class="section-data">' +
                                ' | Questions: '+ no_of_questions +
                                ' | Time: '+ time + '(M)' +
                                '</span>';
                            fields_response += '<span class="rurera-hide">' +
